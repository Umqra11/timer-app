<!--
  ...
  Sekmeler:
    - Kronometre   → /
    - Liderlik     → /leaderboard (D-033 ortanca sekme, D-060 single-room, D-061 /rooms kaldırıldı)
    - Profil       → /profile
-->
<script lang="ts">
	...
	type Tab = {
		href: string;
		label: string;
		icon: 'timer' | 'leaderboard' | 'user';
	};

	const tabs: Tab[] = [
		{ href: '/', label: 'Kronometre', icon: 'timer' },
		{ href: '/leaderboard', label: 'Liderlik', icon: 'leaderboard' },
